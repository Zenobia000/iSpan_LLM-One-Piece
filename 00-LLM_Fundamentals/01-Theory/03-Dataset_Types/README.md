...
為了清晰地展示不同訓練階段與其所需數據集之間的關係，以下總結了各階段的核心數據需求：

*   **1. 預訓練階段 (Pre-training Stage)**
    *   **目標**：建立模型的基礎世界知識和語言能力。
    *   **對應數據體系**：`0.3.1 預訓練數據集體系`
    *   **核心數據類型**：
        *   **大規模通用文本語料**：如 Common Crawl、Wikipedia、Books3，提供廣泛的背景知識。
        *   **代碼語料庫**：如 The Stack，賦予模型程式碼理解與生成能力。

*   **2. 微調階段 (Fine-tuning Stage)**
    *   **目標**：賦予模型特定的任務能力，如指令跟隨或對話。
    *   **對應數據體系**：`0.3.2 微調階段數據集`
    *   **核心數據類型**：
        *   **指令微調數據集**：如 Alpaca、Vicuna，教會模型如何理解並執行人類指令。
        *   **對話系統數據集**：如 Persona-Chat、MultiWOZ，訓練模型進行流暢、有上下文的多輪對話。

*   **3. 對齊與安全階段 (Alignment & Safety Stage)**
    *   **目標**：確保模型的輸出符合人類價值觀（有用、無害、誠實）並具備安全性。
    *   **對應數據體系**：`0.3.3 對齊與安全數據集`
    *   **核心數據類型**：
        *   **人類偏好數據**：如 Anthropic HH-RLHF、SHP，用於訓練獎勵模型，讓模型學會分辨好壞回答。
        *   **安全評估數據集**：如 RealToxicityPrompts、WinoBias，用於檢測和減少模型的毒性內容、偏見及其他安全風險。

### 0.3.1 預訓練數據集體系

#### 一句話心法：餵給模型「全世界的書」，讓它先學會通用的知識和語言。
...
#### 一句話心法：餵給模型「特定領域的教科書和考題」，把它訓練成某個領域的專家。
...
#### 一句話心法：餵給模型「品德教育手冊」，教它分辨好壞，變得有用且無害。
...
#### 一句話心法：使用數據前要三思，確保不偷、不搶、不侵犯別人隱私。
