...
本實驗將帶您親手操作，體驗一個大型語言模型從零到一的完整演進過程：

*   **1. 預訓練 (`階段一`)**
    *   **目標**：建立模型的語言基礎。
    *   **一句話心法**：先打造一個什麼都懂一點的「通才」寶寶模型，讓它學會說話。

*   **2. 指令微調 (`階段二`)**
    *   **目標**：教會模型聽懂並執行指令。
    *   **一句話心法**：把「通才」訓練成「專才」，教它學會看懂題目、精準回答。

*   **3. RLHF (`階段三`)**
    *   **目標**：讓模型的回答更符合人類的偏好與價值觀。
    *   **一句話心法**：請個老師來給「專才」打分數，教它不僅要答對，還要答得好、答得安全。

*   **4. 綜合評估 (`階段四`)**
    *   **目標**：對比模型在各階段的進化，並分析資源成本。
    *   **一句話心法**：回顧模型的成長歷程，看看它進化了多少，以及我們為此付出了多少「學費」。
